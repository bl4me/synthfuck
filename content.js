// Auto Dungeon Synth Remover - Content Script
...
function hideAlbum(albumData) {
  if (albumData.container) {
    ...
    console.log(`🚫 Hidden album: ${albumData.title} by ${albumData.artist}`);
    stats.totalRemoved++;
    return true;
  }
...
async function processAlbumBatch(albumBatch) {
  const promises = albumBatch.map(async (albumData) => {
    try {
      ...
      const isDungeonSynth = await checkAlbumForDungeonSynth(albumData.href);
...
async function processNewAlbums() {
  ...
  try {
    ...
    console.log(`📊 Found ${newAlbumLinks.length} new album links to check`);
...
function setupMutationWatcher() {
  const observer = new MutationObserver((mutations) => {
        ...
        mutation.addedNodes.forEach((node) => {
          if (node.nodeType === Node.ELEMENT_NODE) {
            ...
            const containsAlbumLinks =
              node.querySelector && node.querySelector('a[href*="/album/"]');
    ...
    if (hasNewContent && !isScrolling) {
      console.log("New content detected via DOM changes, processing albums...");
